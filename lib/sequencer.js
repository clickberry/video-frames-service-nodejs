 ...
 * @param      {Function}  fn         Callback.
 */
function uploadFrameToS3(s3Bucket, s3Dir, framePath, fn) {
  var fileStream = fs.createReadStream(framePath);
  var fileName = path.basename(framePath);
...
function processFrame(s3Bucket, s3Dir, file, sequencer) {
  return function (fn) {
    uploadFrameToS3(s3Bucket, s3Dir, file, function (err, uri) {
      if (err) return fn(err);
 ...
 */
Sequencer.prototype.downloadAndExtractToS3 = function (videoUri, s3Bucket, fn) {
  var sequencer = this;
  fn = fn || function (err) {
    if (err) debug(err);
  };
    ...
    downloadFile(videoUri, localVideoPath, function (err) {
      ...
      tmp.dir({unsafeCleanup: true}, function (err, framesPath, cleanupLocalFrames) {
        ...
        extractFrames(localVideoPath, framesPath, function (err, frames) {
          ...
          debug(frames.length + ' frames extracted from video ' + videoUri);

          // upload files to s3 and generate events
          var uploadTasks = [];
          var s3_dir = path.basename(videoUri, path.extname(videoUri));
          frames.forEach(function (file) {
            uploadTasks.push(processFrame(s3Bucket, s3_dir, file, sequencer));
          });
