...
/**
 * Parses frame index from file name: xxx_idx.
 *
 * @param      {string}  file    File path/name.
 * @return     {number}  Parsed frame index.
 */
function getFrameIndex(file) {
  if (!getFrameIndex.pattern) {
    // memoization caching
    getFrameIndex.pattern = /_(\d+)/;
  }

  var fileName = path.basename(file);
  var match = getFrameIndex.pattern.exec(fileName);
      ...
      tmp.dir({unsafeCleanup: true}, function (err, framesPath, cleanupLocalFrames) {
        ...
        extractFrames(localVideoPath, framesPath, function (err, frames) {
          ...
          frames = frames.filter(function (f) {
            var idx = getFrameIndex(f);
            if (idx % idxMultiplier !== 0) return false;
            return true;
          });
